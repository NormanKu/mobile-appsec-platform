...
def test_jadx_adapter_reports_unavailable_when_binary_is_missing(monkeypatch) -> None:
    ...
    assert result == AndroidExternalToolResult(tool_name="jadx", available=False, executed=False)
...
def test_jadx_adapter_extracts_normalized_signals(monkeypatch) -> None:
    ...
    def fake_run(*, executable: str, input_path: Path, output_dir: Path) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(args=[executable, str(input_path)], returncode=0, stdout="", stderr="")
    ...
    assert {"readable_source", "hardcoded_url", "candidate_secret", "naming_pattern"} <= kinds
    assert any("ApiClient" in signal.value for signal in result.signals if signal.kind == "readable_source")
    assert any("staging.example.com" in signal.value for signal in result.signals if signal.kind == "hardcoded_url")
    assert any("API_KEY=abcdef..." == signal.value for signal in result.signals if signal.kind == "candidate_secret")
    assert any("internal" in signal.value.lower() for signal in result.signals if signal.kind == "naming_pattern")
